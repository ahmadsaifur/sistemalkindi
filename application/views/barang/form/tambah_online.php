						...
						<div class="panel-body">
							...
							<div class="form-horizontal">
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<select name='id_herbalis' id='id_herbalis' class='form-control input-sm' style='cursor: pointer;'>
											<option value=''>-- pilih herbalis --</option>
											<?php
											if($herbalis->num_rows() > 0)
											{
												foreach($herbalis->result() as $p)
												{
													echo "<option value='".$p->id_herbalis."'>".$p->nama."</option>";
												}
											}
											?>
										</select>
					...
					<table class='table table-bordered' id='TabelTransaksi'>
						<thead>
							<tr>
								...
								<th style='width:230px;'>Kode Barang</th>
								<th>Nama Barang</th>
...
function BarisBaru()
{
	...
	var Baris = "<tr>";
		...
		Baris += "<td><input type='text' class='form-control' id='discount' name='discount[]' onkeypress='return check_int(event)' disabled></td>";
		Baris += "<td>";
			Baris += "<input type='hidden' name='sub_total[]' id='valtotal'>";
			Baris += "<span></span>";
		Baris += "</td>";
...
$(document).ready(function(){
	...
	var KodeBarang = $('#TabelTransaksi tbody tr:eq('+Indexnya+') td:nth-child(2) input').val();
				...
				var total = parseInt(Harga) * parseInt(discount) / 100;
				var subtotal = parseInt(Harga) - parseInt(total);
