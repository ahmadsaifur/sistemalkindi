...
<div class="container-fluid">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5><i class='fa fa-cube fa-fw'></i> Barang <i class='fa fa-angle-right fa-fw'></i> Obat Gudang <b style='color:red'>KIMIA</b></h5>
			<hr />
			<a href="<?php echo site_url('barang/tambah'); ?>" class="btn btn-success" id='TambahBarang'>Tambah Barang</a>
			<a href="<?php echo site_url('barang/barang_online_herbal'); ?>" class="btn btn-warning"> Gudang Herbal</a>
			<a href="<?php echo site_url('barang/barang_online_kimia'); ?>" class="btn btn-warning"> Gudang Kimia</a>
			<hr>


			<a href="<?php echo site_url('barang/update_stok_online_kimia'); ?>" class="btn btn-primary" id='UpdateStokBarang_kimia'>Update Gudang Kimia</a>

			<?php if ($level == 'admin') { ?>
				<a href="<?php echo site_url('barang/tambah_kadaluarsa_online_kimia'); ?>" class="btn btn-danger" id='UpdateKadaluarsaBarang_kimia'>Tambah Kadaluarsa</a>
			<?php } ?>
...
<script type="text/javascript" language="javascript">
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax": {
				url: "<?php echo site_url('barang/barang_json_online_kimia'); ?>",
				type: "post",
				error: function() {
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#DetailBarang, #TambahBarang, #EditBarang, #UpdateStokBarang, #UpdateStokBarang_kimia, #UpdateKadaluarsaBarang_kimia', function(e) {
		e.preventDefault();
		if ($(this).attr('id') == 'DetailBarang') {
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').addClass('modal-lg');
			$('#ModalHeader').html('Detail Obat Kadaluarsa');
		}
		...
		if ($(this).attr('id') == 'UpdateStokBarang') {
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').removeClass('modal-lg');
			$('#ModalHeader').html('Update Stok Barang');
		}
		if ($(this).attr('id') == 'UpdateStokBarang_kimia') {
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').removeClass('modal-lg');
			$('#ModalHeader').html('Update Stok Barang Kimia');
		}
		if ($(this).attr('id') == 'UpdateKadaluarsaBarang_kimia') {
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').removeClass('modal-lg');
			$('#ModalHeader').html('Update Kadaluarsa Kimia');
		}
