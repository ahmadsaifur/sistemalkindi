...
<div class="container">
    <div class="panel panel-default">
        <div class="panel-body">
            <h5><i class='fa fa-cube fa-fw'></i> Barang <i class='fa fa-angle-right fa-fw'></i> History Gudang Herbal</h5>
            <hr />
...
if ($level == 'admin' or $level == 'inventory') {
    // $tambahan .= nbs(2) . "<a href='" . site_url('barang/history_update_stok_online') . "' class='btn btn-warning'>History Update Stok Obat</a>";
    // $tambahan .= nbs(2) . "<span id='Notifikasi' style='display: none;'></span>";
    $tambahan .= nbs(2) . "<a href='" . site_url('barang/history_brg_herbal') . "' class='btn btn-warning'>History Gudang Herbal </a>";
    $tambahan .= nbs(2) . "<a href='" . site_url('barang/history_brg_kimia') . "' class='btn btn-warning'>History Gudang Kimia</a>";
    $tambahan .= nbs(2) . "<span id='Notifikasi' style='display: none;'></span>";

    // $tambahan .= nbs(2) . "<a href='" . site_url('barang/history_update_stok_online') . "' class='btn btn-warning'>History Gudang Herbal Obat</a>";
    // $tambahan .= nbs(2) . "<a href='" . site_url('barang/history_update_stok_online') . "' class='btn btn-warning'>History Gudang Kimia</a>";
    // $tambahan .= nbs(2) . "<span id='Notifikasi' style='display: none;'></span>";
}
?>
...
<script type="text/javascript" language="javascript">
    $(document).ready(function() {
        var dataTable = $('#my-grid').DataTable({
            ...
            "ajax": {
                url: "<?php echo site_url('barang/barang_json_history_herbal'); ?>",
                type: "post",
                error: function() {
                    $(".my-grid-error").html("");
    ...
    $(document).on('click', '#TambahBarang, #Hapus_historis_Barang', function(e) {
        e.preventDefault();
    ...
    $(document).on('click', '#TambahBarang, #Edit_historis_Barang', function(e) {
        e.preventDefault();
        let id = $(this).attr('href');
        // alert(id);
