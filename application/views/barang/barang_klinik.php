...
<div class="container-fluid">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5><i class='fa fa-cube fa-fw'></i> Obat <i class='fa fa-angle-right fa-fw'></i> Obat Klinik</h5>
			<hr />
			
			<a href="<?php echo site_url('barang/tambah_klinik');?>" class='btn btn-primary' id='TambahBarang'>Update Obat Herbal</a>
			<a href="<?php echo site_url('barang/tambah_klinik_Kimia');?>" class='btn btn-info' id='TambahBarangKimia'>Update Obat Kimia</a>
			<a href="<?php echo site_url('barang/tambah_klinik_baru'); ?>" class='btn btn-success' id='TambahKlinikBaru'>Tambah Obat</a>
				...
				<table id="my-grid" class="table table-striped table-bordered">
					<thead>
						<tr>
							...
							<th>Harga</th>
							<th>Keterangan</th>
							<?php if($level == 'admin' OR $level == 'inventory' OR $level == 'kasir') { ?>
...
<?php
$tambahan = '';
if($level == 'admin' OR $level == 'inventory'OR $level == 'kasir')
{
	/*$tambahan .= nbs(2)."<a href='".site_url('barang/tambah_klinik')."' class='btn btn-info' id='TambahBarang'>Tambah Barang</a>";*/
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('barang/barang_json_klinik'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#HapusBarang', function(e){
		...
		$('#ModalContent').html('Apakah anda yakin ingin menghapus <br /><b>'+$(this).parent().parent().find('td:nth-child(4)').html()+'</b> ?');
	...
	$(document).on('click', '#TambahBarang, #TambahKlinikBaru, #EditBarang, #TambahBarangKimia', function(e){
		e.preventDefault();
		if($(this).attr('id') == 'TambahBarang')
		{
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').addClass('modal-lg');
			$('#ModalHeader').html('Tambah Barang HERBAL');
		}

		if($(this).attr('id') == 'TambahKlinikBaru')
		{
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').addClass('modal-lg');
			$('#ModalHeader').html('Tambah Barang KLINIK');
		}
		
		if($(this).attr('id') == 'TambahBarangKimia')
		{
			$('.modal-dialog').removeClass('modal-sm');
			$('.modal-dialog').removeClass('modal-lg');
			$('#ModalHeader').html('Tambah Barang KIMIA');
		}
