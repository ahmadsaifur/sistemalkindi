...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-body">
			<h5 style="color:red"><i class='fa fa-shopping-cart fa-fw'></i> Penjualan <i class='fa fa-angle-right fa-fw'></i> History Asuransi Acc</h5>
			<hr />
			
			<a href="<?php echo site_url('penjualan/history_asuransi');?>" type="button" class="btn btn-primary">Transaksi OK</a>
			<a href="<?php echo site_url('penjualan/history_asuransi_acc'); ?>" type="button" class="btn btn-success">Transaksi Pending</a>
...
<script type="text/javascript" language="javascript" >
	$(document).ready(function() {
		var dataTable = $('#my-grid').DataTable( {
			...
			"ajax":{
				url :"<?php echo site_url('penjualan/history-json_apotek_asuransi'); ?>",
				type: "post",
				error: function(){ 
					$(".my-grid-error").html("");
	...
	$(document).on('click', '#HapusTransaksi', function(e){
		...
		var Check = "<br /><hr style='margin:10px 0px 8px 0px;' /><div class='checkbox'><label><input type='checkbox' name='reverse_stok' value='yes' id='reverse_stok'> Acc stok barang</label></div>";*/
		$('.modal-dialog').removeClass('modal-lg');
		$('.modal-dialog').addClass('modal-sm');
		$('#ModalHeader').html('Konfirmasi');
		$('#ModalContent').html('Apakah anda yakin ingin acc transaksi <b>'+$(this).parent().parent().find('td:nth-child(3)').text()+'</b> ?' /*+ Check*/);
